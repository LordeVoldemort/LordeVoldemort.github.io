  ...
  options: {
    ...
    scales: {
      yAxes: [{
        ...
        //Returns a label if it is a power of ten
      return (label == 10 || label == 100 || label == 1000 || label == 10000 || label == 100000 || label == 1000000 || label == 10000000) ? label/1000+'k' : '';
          }
        },
        scaleLabel: {
          display: true,
...
function loadContent() {
  ...
  let storeDate = new Date(covidJsObj.Date);
  console.log("Date Stored: "+storeDate);
  console.log("Current Date: "+dayjs().$d);
  //Check if the data is at least 24 old
  if( (storeDate.getDate() == dayjs().date()) && (storeDate.getHours() <= dayjs().hour())){
    //Gets data, builds and then displays chart
    loadData(); 
  }
  else {
    console.log("Out of Date Data ... Getting");
    AJAXcall();
  }
...
function loadData () {
    ...
    //Order the elements in the array by TotalConfirmedPer100000 in desending order
    newConfirmedOver1000 = _.orderBy(newConfirmedOver1000, ['TotalConfirmedPer100000'], ['desc']);
    //Filter for total deaths over 50,000
    newConfirmedOver1000 = newConfirmedOver1000.filter((x) => {return x.TotalDeaths >= 50000});
    ...
    chartData.data.datasets[2].backgroundColor = "rgba(50,60,210,0.4)"; // green
    
      chartData.data.datasets[0].label = 'Total Cases';
      chartData.data.datasets[1].label = 'Total Deaths';
    chartData.data.datasets[2].label = 'Total Cases Per 10,000';
    
      chartData.data.labels = newConfirmedOver1000.map( (x) => x.Slug );
    
      chartData.data.datasets[0].data = newConfirmedOver1000.map((x) => x.TotalConfirmed );
      chartData.data.datasets[1].data = newConfirmedOver1000.map((x) => x.TotalDeaths );
    chartData.data.datasets[2].data  = newConfirmedOver1000.map((x) => x.TotalConfirmedPer100000);
    
      chartData.options.title.text = "Covid 19 Hotspots " + dayjs().toString();
